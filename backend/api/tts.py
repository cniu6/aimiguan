...
from core.database import get_db, AITTSTask
...
@router.post("/tasks")
async def create_tts_task(req: TTSRequest, db: Session = Depends(get_db)):
    """创建 TTS 任务"""
    task = AITTSTask(
...
async def get_tts_task(task_id: int, db: Session = Depends(get_db)):
    """获取 TTS 任务状态"""
