...
- [x] U-4 响应式与可访问性验收：已补齐 `375/768/1024/1440` 断点样式、自定义块焦点可见性、移动端操作区栈叠与基础语义属性。
- [x] U-5 动效规范收敛：已将非强制模式切换的页面交互动效收敛到 `150~300ms`，并统一为 `transform/opacity` 过渡。
- [x] C-1 内容表达规范：所有关键卡片必须包含“当前值 + 环比/趋势 + 风险解释 + 下一步建议”，禁止只显示数字。
- [x] C-2 业务文案规范：已统一防御/探测页术语为“事件/任务/发现/处置”，并移除“系统/模块”泛化描述，改为可执行动作表达。
