...
> 以下规则为系统核心设计约束，所有开发必须严格遵守，不得以任何理由绕过。

### RULE-01：系统运行模式定义（禁止误用"主动/被动"开关）

**防御坚守是系统的唯一持续默认状态，不存在"主动防御/被动监控"二选一的系统级开关。**

- **防御坚守（始终自动运行）**：后台持续采集 HFish 蜜罐告警 → AI 评分 → 人工审批 → 交换机封禁执行。该链路由 `scheduler_service` 自动驱动，无需任何手动开启，也不能被关闭。
- **主动探测（手动触发）**：Nmap 扫描、漏洞检测等主动探测任务，必须由用户在**「主动探测」面板 → 扫描管理**中手动添加任务来触发。不存在"切换到主动模式"的系统级开关，避免资源持续消耗。
- **禁止**在任何页面（包括 `SettingsPage`）提供"系统运行模式 PASSIVE/ACTIVE 二选一"的切换控件，该控件已被移除。
- **后台调度器**（`scheduler_service.py`）中的 Nmap 定时扫描是按配置间隔自动执行的"定期巡检"，每次执行均创建 `ScanTask` 记录，与用户手动触发的任务共用同一套追溯体系。

### RULE-02：顶栏模式切换动画（强制触发，无例外）

**登录后，无论当前处于哪个页面，点击顶栏「防御坚守 / 主动探测」切换时，必须触发完整的 GSAP 切换动画，并在动画结束后跳转到目标模式的默认页。**

- **不得**在中性页面（`/settings`、`/integrations`、`/audit`、`/profile` 等）跳过动画或仅更新状态而不跳转。
- 动画实现位于 `frontend/src/components/Layout.vue` 的 `runModeTransition()` 函数，该函数对所有页面均适用。
- 切换动画颜色规范：防御坚守 → 蓝色（`#3b82f6`）；主动探测 → 橙色（`#f97316`）。
- 切换仅影响前端展示面板与路由跳转，**不触发任何后端运行状态变更**。

### RULE-03：后台自动任务全程可追溯

**所有由 `scheduler_service` 自动执行的任务，必须将执行结果写入 `AuditLog` 表（actor="scheduler"），确保自动任务与手动任务同等可审计。**

- HFish 自动同步：action=`hfish_auto_sync`，写入成功/失败结果。
- Nmap 自动扫描：action=`nmap_auto_scan`，通过 `nmap_scanner.scan_target` 创建 `ScanTask` 记录，并额外写入 `AuditLog`。
- 自动任务的 `trace_id` 格式：`hfish_sync_<8位hex>` / `nmap_auto_<8位hex>`，与手动任务区分。
...
- 视图定义
  - `防御坚守`：展示防御监控链路与威胁处置面板（始终运行，不可关闭）。
  - `主动探测`：展示探测扫描链路与任务结果面板（手动添加任务触发）。
- 切换控件
  - Topbar 显示双视图切换：`防御坚守 | 主动探测`。
  - **该切换仅影响前端展示面板与路由跳转，不切换后端任何运行状态。**
- 切换动画（强制，见 RULE-02）
  - 无论当前在哪个页面（包括 settings/integrations/audit 等中性页面），切换时必须触发完整 GSAP 动画。
  - 动画结束后自动跳转到目标模式的默认页（防御坚守 → `/defense/dashboard`；主动探测 → `/probe/dashboard`）。
  - 禁止在任何页面跳过动画或仅更新状态而不跳转。
- 运行说明
  - 审计日志、告警流、任务执行等后端链路持续实时运行，不因前端视图切换而停止。
  - 主动探测任务需在主动探测面板手动添加扫描任务，不存在「系统级主动模式开关」（见 RULE-01）。
...
#### D10. 顶栏与右上角详细交互时序
- `视图切换时序`（强制，见 RULE-02）
  1. 用户点击 Topbar 视图切换（任何页面均有效）。
  2. 立即触发 GSAP 全屏切换动画（防御坚守蓝色 / 主动探测橙色）。
  3. 动画进行中（约 1.2s 时）后台执行路由跳转到目标模式默认页。
  4. 动画结束，新面板呈现，后端审计与任务链路持续实时运行。
  5. 不触发后端任何运行状态变更。
- `退出登录时序`
  1. 用户点击右上角头像菜单 `退出登录`。
  2. 弹窗确认。
...
#### 成品化体验与内容要求（新增 TODO）
- [x] U-1 视觉系统升级：重构 `frontend/src/style.css` 主题 token（主色、语义色、层级阴影、圆角），形成统一“安全运营控制台”视觉语言，减少当前通用模板感。
- [x] U-2 仪表盘信息层级重构：将“核心风险指标 / 待处置任务 / 趋势洞察 / 操作入口”分区，避免纯卡片平铺导致重点不突出。
- [x] U-3 空态/加载态/异常态规范：每个模块补齐 Skeleton、空态引导和错误恢复操作（重试/查看日志），禁止仅显示“暂无数据”。
- [ ] U-4 响应式与可访问性验收：按 `375/768/1024/1440` 断点逐页验收，补齐键盘焦点可见性与色彩对比度（>=4.5:1）。
- [ ] U-5 动效规范收敛：除强制模式切换动画外，页面交互动效统一在 `150~300ms`，仅使用 `transform/opacity`，避免过度特效影响性能。
- [x] C-1 内容表达规范：所有关键卡片必须包含“当前值 + 环比/趋势 + 风险解释 + 下一步建议”，禁止只显示数字。
- [ ] C-2 业务文案规范：统一术语（事件/任务/发现/处置），减少“系统/模块”泛化描述，文案需直接指向可执行动作。
- [x] D-1 演示数据基线：提供一键导入演示数据脚本（防御 + 探测 + AI + 审计，命令：`python backend/seed_demo_data.py`），避免新环境全零数据影响体验评估。
- [x] D-2 链路状态真实性：去除前端硬编码健康状态（如固定 `ok: true`），改为后端实时指标驱动。
- [ ] F-1 AI 能力去占位：将 `ai_chat/report/analyze_scan_result` 从模板或占位返回替换为真实模型调用，并记录降级原因与 trace_id。
- [ ] F-2 联动能力去模拟：将推送测试与 TTS 处理从模拟执行替换为真实通道调用（保留沙箱开关与审计）。
...
### Step 8：可观测性与告警
- [x] 8.1 统一结构化日志字段（含 `trace_id`）。
- [x] 8.2 接入核心指标：成功率、延迟、重试率、失败类型分布。
- [x] 8.3 配置告警阈值（MCP 失败率、防火墙回执失败率、扫描超时率）。
- [x] 8.4 搭建基础看板（防御、扫描、外部依赖、AI 可用性）。
- [x] 8.5 验证：人为制造失败场景，告警能在阈值内触发。

### Step 9：测试与 CI 门禁
- [x] 9.1 单元测试：策略引擎、状态机、解析器。
- [x] 9.2 集成测试：API + SQLite + Mock MCP/防火墙。
- [x] 9.3 E2E：防御闭环 1 条、扫描闭环 1 条。
- [ ] 9.4 前端关键交互测试：审批、状态刷新、错误提示。
- [ ] 9.5 新增后台壳 E2E：Sidebar 路由切换、Topbar 视图切换、右上角退出登录。
- [x] 9.6 新增权限 E2E：viewer/operator/admin 菜单可见性与接口访问控制。
- [ ] 9.7 新增系统功能 E2E：通知已读、全局搜索跳转、探测任务触发审计写入。
- [x] 9.8 CI 接入：lint + test + build + e2e。
- [x] 9.9 阻断规则：任一关键流程失败禁止合并。
